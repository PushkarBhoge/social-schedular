...
interface PlatformPickerModalProps{
    ...
    onClose: () => void
    onConnect: (platfromId: string) => void
}
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-100">
            ...
            <div className="p-6 flex flex-col gap-2">
                {PLATFORMS.map((p)=>{
                    ...
                    return (
                        <button key={p.id} disabled={isConnected || isConnecting} onClick={()=> onConnect(p.id)} className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-red-200 bg-red-50 cursor-default" : "border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}>
                            {/* icon */}
                            <p.icon className={`size-5 shrink-0 ${isConnected ? "text-red-600" : "text-slate-500"}`}/>
                            ...
                            {/* status */}
                            {isConnected && <CheckCircleIcon className="size-4 text-red-500 shrink-0 ml-auto"/>}
                            {isConnecting && <div className="size-4 border-2 border-red-600 border-t-transparent rounded-full animate-spin shrink-0 ml-auto"/>}
                            {!isConnected && !isConnecting && <ExternalLinkIcon className="size-3.5 text-slate-400 shrink-0 ml-auto"/>}
                        </button>
                    )
                })}
            </div>
