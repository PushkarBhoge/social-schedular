...
const AccountList = ({accounts, onDisconnect}: AccountListProps) => {
    ...
    const handleDisconnect = async (accountId: string) => {
        const confirm = window.confirm("Are you sure yount wnat to disconnect this account?");
        if(!confirm) return;
        ...
        {accounts.map((account, index)=>{
            ...
            return (
                <div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 transition-all">
                    ...
                    <button onClick={()=> handleDisconnect(account._id)} title="Disconnnect account" className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-red-500 transition-all">
