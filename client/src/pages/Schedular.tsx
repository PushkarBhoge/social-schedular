...
import { ArrowRightIcon, Calendar1Icon, CalendarDaysIcon, CalendarIcon, ClockIcon, SendIcon, XIcon } from "lucide-react"
      ...
      <div className="w-full lg:w-[460px] shrink-0">
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center gap-2 mb-6">
            <h2 className="text-lg text-slate-700 ">Composed Post</h2>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleSchedule}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {PLATFORMS.map((p)=>{
                  ...
                  return (
                    <button key={p.id} type="button" onClick={()=> togglePlatform(p.id)} className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-red-50 border-red-300 text-red-500 scale-103" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}>
            ...
            <div>
              ...
              {mediaFile ? (
                ...
                <label className="flex items-center justify-center gap-2 p-5 py-10 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-red-300 hover:bg-red-50/30 transition-all group">
                  ...
                  <input type="file" accept="immage/*,video/*" className="hidden" onChange={(e)=>e.target.files?.[0] && setMediaFile(e.target.files[0])}/>
                </label>
              )}
            </div>
            ...
            {/* submit */}
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3.5 bg-red-500 hover:bg-red-500 transition-all text-white rounded-lg">
              {loading ? (
                <>
