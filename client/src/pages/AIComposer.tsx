...
const AIComposer = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-red-200 transition-all relative overflow-hidden">
              <div className="flex flex-col h-full space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button onClick={()=> setActiveSchedular(gen)} className="flex-1 bg-slate-100 hover:bg-red-500 hover:text-white text-slate-600 text-xs py-2.5 rounded-lg transition-all">
                    Schedule Post
                  </button>
                </div>
              </div>
...

          {
            ...
            <div className="col-span-full py-20 text-center space-y-2">
              ...
              <p className="text-slate-400 text-sm">No content generations yet. Try generating some content using AI.</p>
            </div>
            )
      ...
      {/* schedular model */}
      {activeSchedular && (
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30">
              ...
              <button onClick={()=> setActiveSchedular(null)} className="p-2 rounded-full hover:bg-slate-100 text-slate-400 transition-colors"><XIcon className="size-5"/></button>
            </div>

            <div className="flex-1 overflow-y-auto p-8 space-y-4">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4 ">
                <p className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">{activeSchedular.prompt}</p>
              </div>

              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4 ">
                <p className="text-slate-800 text-sm leading-relaxed whitespace-pre-wrap">{activeSchedular.content}</p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PLATFORMS.map((p)=>{
                      ...
                      return (
                        <button key={p.id} onClick={()=> setSelectedPlatforms((prev)=> (prev.includes(p.id) ? prev.filter((x)=> x !== p.id) : [...prev, p.id]))} className={`p-2.5 rounded-md border text-xs ${active ? "bg-red-500/80 text-white border" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}>
